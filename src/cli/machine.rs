    ...
    pub fn run(self) -> smolvm::Result<()> {
        ...
        if let Some(ref img) = image {
                    ...
                    let persist_result = SmolvmConfig::load().and_then(|mut config| {
                        vm_common::persist_named_running(
                            ...
                            Some(DefaultVmOverrides {
                                ...
                                image_user: defaults.user.clone(),
                                entrypoint: Vec::new(),
                                cmd: command.clone(),
            ...
            if self.detach {
                ...
                if !is_idle {
                    let pid =
                        client.vm_exec_background(command, env.clone(), params.workdir.clone())?;
...
// ============================================================================
// Exec Command (Persistent)
// ============================================================================

/// Execute a command in a persistent machine.
///
/// Image-backed machines run through the image rootfs. Bare machines run
/// directly in the VM rootfs.
///
/// Examples:
...
fn print_exec_event(event: smolvm::agent::ExecEvent) {
    match event {
    ...
    pub fn run(self) -> smolvm::Result<()> {
        ...
        if let Some(ref image) = record_image {
            ...
            if self.stream {
                ...
                let exit_code = client.run_streaming_with(config, |event| {
            ...
            if self.stream {
                let exit_code = client.vm_exec_streaming_with(
                    self.command.clone(),
                    env.clone(),
                    workdir.clone(),
                    self.timeout,
    ...
    pub fn run(self) -> smolvm::Result<()> {
        ...
        db.update_vm(&self.name, |r| {
            ...
            if self.no_net {
                ...
                r.network = false;
                // Clear egress policy — allow_cidrs and allow_hosts imply
                ...
                if r.egress_policy_hosts.is_some() {
                    changes.push("  cleared allow_hosts".to_string());
                    r.egress_policy_hosts = None;
                }
            }
